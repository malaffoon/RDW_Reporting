...
export class GroupAssessmentService {
  ...
  getMostRecentAssessment(search: Search) {
    if (search.schoolYear == null) {
      return this.filterOptionService
        .getExamFilterOptions()
        .pipe(
          mergeMap(({ schoolYears }) =>
            this.getRecentAssessmentBySchoolYear(
  ...
  getAvailableAssessments(search: Search) {
    return this.dataService
      .get(`${ServiceRoute}/assessments`, {
        params: <any>search
      })
      .pipe(
        catchError(ResponseUtils.badResponseToNull),
        map(serverAssessments =>
          this.mapper.mapAssessmentsFromApi(serverAssessments)
        )
      );
  }
  ...
  getTargetScoreExams(search: ExamSearch) {
    return this.dataService
      .get(`${ServiceRoute}/examsWithTargetScores`, {
        params: <any>search
      })
      .pipe(
        catchError(ResponseUtils.badResponseToNull),
        map(serverTargetScoreExams =>
          this.mapper.mapTargetScoreExamsFromApi(serverTargetScoreExams)
        )
      );
  }
  ...
  getAssessmentItems(search: ExamItemSearch) {
    return this.dataService
      .get(`${ServiceRoute}/examitems`, {
        params: <any>search
      })
      .pipe(
        catchError(ResponseUtils.badResponseToNull),
        map(serverAssessmentItems =>
          this.mapper.mapAssessmentItemsFromApi(serverAssessmentItems)
        )
      );
  }
  ...
  private getRecentAssessmentBySchoolYear(search: Search) {
    return this.dataService
      .get(`${ServiceRoute}/latestassessment`, {
        search: <any>search
      })
      .pipe(
        catchError(ResponseUtils.badResponseToNull),
