...
export class UserGroupAssessmentProvider implements AssessmentProvider {
  ...
  getAssessmentItems(assessmentId: number, itemTypes?: string[]): Observable<AssessmentItem[]> {
    ...
    return this.service.getAssessmentItems({userGroupId: group.id, schoolYear, assessmentId, itemTypes});
  }

  getAvailableAssessments(): Observable<Assessment[]> {
    const { group, schoolYear } = this.stateProvider;
    return this.service.getAvailableAssessments({userGroupId: group.id, schoolYear});
  }
