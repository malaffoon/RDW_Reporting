...
const PerformanceLevelColorsByAssessmentTypeCode: Map<
  string,
  string[]
> = new Map([['iab', Pallets[0]], ['ica', Pallets[1]], ['sum', Pallets[2]]]);

const PerformanceLevelColorsNumberOfPerformanceLevels: Map<
  number,
  string[]
> = new Map([[3, Pallets[0]], [4, Pallets[1]]]);
...
export class ColorService {
  ...
  getPerformanceLevelColorsByAssessmentTypeCode(
    code: string,
   ...
   */
  // TODO:ConfigurableSubjects drive through subject service
  getPerformanceLevelColorsByNumberOfPerformanceLevels(
    levels: number,
    performanceLevel: number
  ): string {
    return 'gray-darkest';
