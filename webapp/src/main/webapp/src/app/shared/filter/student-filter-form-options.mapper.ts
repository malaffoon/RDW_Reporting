...
export class StudentFilterFormOptionsMapper {
  ...
  fromOptions(options: StudentFilterOptions): StudentFilterFormOptions {
    ...
    return <StudentFilterFormOptions>{
      ...
      ethnicities: options.ethnicities.map(
        optionMapper(
          value => translate(`common.ethnicity.${value}`),
          value => `Ethnicity: ${value}`
        )
      ),
      englishLanguageAcquisitionStatuses: options.englishLanguageAcquisitionStatuses.map(
        optionMapper(
          value => translate(`common.elas.${value}`),
          value => `English Language Acquisition Status: ${value}`
        )
      ),
      individualEducationPlans: options.individualEducationPlans.map(
        optionMapper(
      ...
      migrantStatuses: options.migrantStatuses.map(
        optionMapper(
          value => translate(`common.boolean.${value}`),
          value => `Migrant Status: ${value}`
        )
      ),
      militaryConnectedCodes: options.militaryConnectedCodes.map(
        optionMapper(
