  ...
  template: `
    <div class="btn-group btn-block" [ngClass]="{ open: open }">
      ...
      >
        ...
        <i class="fa fa-ellipsis-v" [ngClass]="{ 'mr-xs': hasText }"></i>
        {{ text }}
      </button>
      <ul *ngIf="open" class="dropdown-menu" role="menu">
        <li *ngFor="let action of actions" role="menuitem">
          <ng-container
            *ngTemplateOutlet="actionTemplate; context: { action: action }"
          ></ng-container>
        </li>
      </ul>
    </div>
    <ng-template #actionTemplate let-action="action">
      <a
        ...
        triggers="{{
          action.tooltip(item) == '' ? '' : 'mouseenter:mouseleave'
        }}"
        placement="right"
        container="body"
...
export class PopupMenuComponent {
  ...
  public getSubActions(action: PopupMenuAction): PopupMenuAction[] {
    if (!this._subActions.has(action)) {
      ...
      action
        .getSubActions(this.item)
        .subscribe((subActions: PopupMenuAction[]) => {
