...
export class StudentExamHistoryService {
  ...
  findOneById(id: number): Observable<StudentExamHistory> {
    return this.dataService.get(`${ServiceRoute}/students/${id}/exams`).pipe(
      ...
      map(serverExamHistory => {
        ...
        history.student = this.assessmentMapper.mapStudentFromApi(
          serverExamHistory.student
        );
  ...
  private mapExamWrappers(
    serverExamWrappers: any
  ): StudentHistoryExamWrapper[] {
    return (serverExamWrappers || [])
      .sort((a, b) =>
        new Date(a.exam.dateTime) > new Date(b.exam.dateTime) ? -1 : 1
      )
  ...
  private mapExamWrapper(serverExamWrapper: any): StudentHistoryExamWrapper {
    ...
    wrapper.assessment = this.assessmentMapper.mapAssessmentFromApi(
      serverExamWrapper.assessment
    );
