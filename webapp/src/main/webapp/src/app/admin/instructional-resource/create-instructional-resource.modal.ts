...
export class CreateInstructionalResourceModal implements OnDestroy {
  ...
  constructor(
    ...
    }).pipe(mergeMap((token: string) => this.findAssessments(token)));

    this.organizationSource = Observable.create((observer: any) => {
      observer.next(this.organizationSearch);
    }).pipe(mergeMap((token: string) => this.findOrganizations(token)));
  ...
  findAssessments(search: string): Observable<Assessment[]> {
    ...
    return this.assessmentService
      .find(query)
      .pipe(map(this.removeDuplicateNames));
  }

  onAssessmentSelect(assessment: Assessment): void {
    this.assessment = assessment;
    this.performanceLevels = this.getPerformanceLevels(
  ...
  valid(): boolean {
    return (
      !Utils.isNullOrUndefined(this.assessment) &&
  ...
  private validateExisting(): ValidationErrors | null {
    ...
    const existingResource = this.existingResources.find(
  ...
  private getPerformanceLevels(
    subject: string,
    assessmentType: string
  ): number[] {
    const subjectDefinition = this.subjectDefinitions.find(
      x => x.assessmentType == assessmentType && x.subject == subject
    );
