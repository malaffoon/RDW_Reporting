...
export class InstructionalResourceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.service.findAll().subscribe((resources: InstructionalResource[]) => {
    ...
    this.filteredResources = this.resources.filter(
      resource =>
        resource.assessmentName
          .toUpperCase()
          .indexOf(this.searchTerm.toUpperCase()) >= 0 ||
        resource.assessmentLabel
          .toUpperCase()
          .indexOf(this.searchTerm.toUpperCase()) >= 0 ||
  ...
  private createActions(): PopupMenuAction[] {
    ...
    updateAction.displayName = () =>
      this.translateService.instant('update-instructional-resource-modal.link');
    updateAction.perform = (resource: InstructionalResource) =>
      this.openUpdateResourceModal(resource);

    const deleteAction: PopupMenuAction = new PopupMenuAction();
    deleteAction.displayName = () =>
      this.translateService.instant('delete-instructional-resource-modal.link');
    deleteAction.perform = (resource: InstructionalResource) =>
      this.openDeleteResourceModal(resource);
