...
export class GroupService {
  ...
  getFilterOptions(): Observable<GroupFilterOptions> {
    return this.dataService
      ...
      .pipe(map(this.mapFilterOptionsFromApi.bind(this)));
  }

  getGroups(query: GroupQuery): Observable<Group[]> {
    return this.dataService
      .get(`${ResourceContext}`, { search: this.mapQueryToParams(query) })
      .pipe(map(groups => groups.map(GroupService.mapGroupFromApi)));
  }
  ...
  private mapQueryToParams(query: GroupQuery) {
    ...
    let subjects =
      query.subject == null ? query.availableSubjects : [query.subject];
  ...
  private mapFilterOptionsFromApi(apiModel): GroupFilterOptions {
    ...
    uiModel.schools =
      (apiModel.schools &&
        apiModel.schools.map(apiSchool => {
