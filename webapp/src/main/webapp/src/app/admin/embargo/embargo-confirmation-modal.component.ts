  ...
  template: `
    <div class="modal-header">
      <h4 class="modal-title pull-left">
        {{ content.header | translate: content.parameters }}
      </h4>
      <button
        type="button"
    ...
    <div class="modal-footer">
      <div class="pull-right">
        <button class="btn btn-xs btn-default" (click)="modal.hide()">
          {{ content.decline | translate: content.parameters }}
        </button>
        <button class="btn btn-xs btn-primary" (click)="accept()">
          {{ content.accept | translate: content.parameters }}
        </button>
      </div>
    </div>
  `
...
export class EmbargoConfirmationModal {
   ...
   */
  content: EmbargoConfirmationModalContent = <
    EmbargoConfirmationModalContent
  >{};
  ...
  private createContent(
    event: EmbargoToggleEvent
  ): EmbargoConfirmationModalContent {
    const [stateEmbargo, stateEnabled] =
      event.embargo.organization.type === OrganizationType.State
    ...
    return {
      ...
      stateDescription: `embargo-confirmation-modal.state-description.${
        event.scope
      }.${stateEnabled}`,
      commandDescription: `embargo-confirmation-modal.command-description.${
        event.embargo.organization.type
      }.${event.scope}.${event.value}`,
      ...
      parameters: {
        ...
        resultCount: Object.keys(event.embargo.examCountsBySubject).reduce(
          (count, key) => count + event.embargo.examCountsBySubject[key],
          0
        )
      }
    };
