...
export class ItemScoringGuideMapper {
  mapFromApi(apiModel: any): ItemScoringGuide {
    ...
    if (
      !Utils.isNullOrUndefined(apiModel.answerKey) &&
      !Utils.isNullOrUndefined(apiModel.answerKey.value)
    )
      uiModel.answerKeyValue = apiModel.answerKey.value;

    if (!Utils.isNullOrUndefined(apiModel.exemplars))
      uiModel.exemplars = apiModel.exemplars.map(exemplar =>
        this.mapScoringCriterionFomApi(exemplar)
      );

    if (!Utils.isNullOrUndefined(apiModel.rubrics))
      uiModel.rubrics = apiModel.rubrics.map(rubric =>
        this.mapScoringCriterionFomApi(rubric)
      );
