...
import { LongitudinalReportQuery } from '../../report/report';
import { isNullOrEmpty } from '../../shared/support/support';

const rowYearAscending = ordering(byNumber).on<AggregateReportRow>(
  row => row.assessment.examSchoolYear
).compare;
const assessmentGradeAscending = ordering(byNumber).on<Assessment>(
  assessment => assessment.gradeSequence
).compare;
const assessmentYearDescending = ordering(byNumber)
  .on<Assessment>(assessment => assessment.schoolYear)
  .reverse().compare;
...
export class LongitudinalCohortChartMapper {
   ...
   */
  fromReport(
    query: LongitudinalReportQuery,
    report: LongitudinalReport,
    measuresGetter: (row: AggregateReportRow) => AggregateReportRowMeasure,
    subjectDefinition: SubjectDefinition
  ): LongitudinalCohortChart {
    if (report.rows.length === 0 || report.assessments.length === 0) {
      return { performanceLevels: [], organizationPerformances: [] };
    }
    return {
      organizationPerformances: this.createOrganizationPerformances(
        query,
        report.assessments,
        report.rows,
        measuresGetter
      ),
      performanceLevels: this.createPerformanceLevels(
        report.assessments,
        subjectDefinition
      )
    };
  ...
  private createOrganizationPerformances(
    query: LongitudinalReportQuery,
    assessments: Assessment[],
    rows: AggregateReportRow[],
    measuresGetter: (row: AggregateReportRow) => AggregateReportRowMeasure
  ): OrganizationPerformance[] {
    const performanceByOrganizationSubgroup: Map<
      string,
      OrganizationPerformance
    > = new Map();
    ...
    const keyGenerator = isNullOrEmpty(query.subgroups)
      ? row =>
          `${row.organization.id}:${row.organization.organizationType}:${
            row.dimension.type
          }:${row.dimension.code}`
      : row =>
          `${row.organization.id}:${row.organization.organizationType}:${
    ...
    const yearGradeScaleScoresTemplate: YearGradeScaleScore[] = [].fill(
      {},
      0,
      years.length
    );
      ...
      .forEach((row: AggregateReportRow) => {
        ...
        if (performance == null) {
          performance = <OrganizationPerformance>{
            ...
            subgroup: this.subgroupMapper.fromAggregateReportRow(
              query,
              row,
              overall
            ),
        ...
        performance.yearGradeScaleScores[
          years.indexOf(yearGradeScaleScore.yearGrade.year)
        ] = yearGradeScaleScore;
      });
  ...
  private createPerformanceLevels(
    assessments: Assessment[],
    subjectDefinition: SubjectDefinition
  ): PerformanceLevel[] {
          ...
          if (performanceLevel != null) {
            ...
            performanceLevels.push(<PerformanceLevel>{
              ...
              name: this.translate.instant(
                ...
                }.asmt-type.${assessmentType}.level.${level}.short-name`
              ),
              color: this.translate.instant(
                `subject.${
                  subjectDefinition.subject
                }.asmt-type.${assessmentType}.level.${level}.color`
              ),
